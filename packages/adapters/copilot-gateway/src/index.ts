...
Built-in GitHub Copilot Chat gateway:
- Paperclip ships with a built-in Copilot Chat gateway at /copilot-gateway on the Paperclip server.
- The gateway proxies requests to GitHub Copilot Chat API (api.individual.githubcopilot.com) using a GitHub token.
- Set url to http://<paperclip-host>:<port>/copilot-gateway to use the built-in gateway without running a separate service.
- Configure the GitHub token via PAPERCLIP_COPILOT_GATEWAY_GITHUB_TOKEN, GH_TOKEN, or GITHUB_TOKEN environment variables.
- The token must have the copilot scope on GitHub.
- Optionally override the Copilot API endpoint via PAPERCLIP_COPILOT_GATEWAY_API_BASE (for GitHub Enterprise).
- Secure the built-in gateway with PAPERCLIP_COPILOT_GATEWAY_TOKEN (optional bearer token required by callers).

Core fields:
- url (string, required): Gateway base URL (http:// or https://)
- authToken (string, optional): Bearer token sent as Authorization header
- model (string, optional): Model ID to request from the gateway (e.g. gpt-4o, claude-3.5-sonnet)
