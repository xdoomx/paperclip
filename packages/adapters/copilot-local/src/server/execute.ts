...
function hasTrustBypassArg(args: string[]): boolean {
  return args.some((a) => a === "--yolo" || a === "--trust" || a === "-f" || a === "--force");
}

function copilotSkillsHome(): string {
  const home = process.env.HOME || process.env.USERPROFILE || "";
  return path.join(home, ".config", "gh", "skills");
}

type EnsureCopilotSkillsInjectedOptions = {
  skillsHome?: string;
  skillsEntries?: Array<{ name: string; source: string }>;
  linkSkill?: (source: string, target: string) => Promise<void>;
};

export async function ensureCopilotSkillsInjected(
  onLog: AdapterExecutionContext["onLog"],
  options: EnsureCopilotSkillsInjectedOptions = {},
) {
  const skillsEntries =
    options.skillsEntries ?? (await listPaperclipSkillEntries(__moduleDir));
  if (skillsEntries.length === 0) return;

  const skillsHome = options.skillsHome ?? copilotSkillsHome();
  try {
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const commandNotes = (() => {
    ...
    notes.push("Prompt is piped to Copilot via stdin.");
  ...
  const buildArgs = (resumeSessionId: string | null): string[] => {
    const args = ["copilot", "agent", "-p", "--output-format", "stream-json", "--workspace", cwd];
  ...
  const runAttempt = async (resumeSessionId: string | null) => {
    ...
    const flushStdoutChunk = async (chunk: string, finalize = false) => {
      ...
      for (const line of lines) {
        ...
        if (trimmed) await onLog("stdout", `${trimmed}\n`);
      }

      if (finalize && stdoutLineBuffer.trim()) {
        await onLog("stdout", `${stdoutLineBuffer.trim()}\n`);
        stdoutLineBuffer = "";
      }
    };
  ...
  const toResult = (
    ...
    const fallbackErrorMessage =
      parsedError || stderrLine || `Copilot exited with code ${attempt.proc.exitCode ?? -1}`;
